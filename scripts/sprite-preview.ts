// Renders every registered pixel sprite (all frames) into a single contact
// sheet PNG for visual review: `pnpm tsx scripts/sprite-preview.ts`.
// Output: release/sprite-preview.png
...
const arrowRow = 1
const sheetH = (rows + arrowRow) * (cell + GAP) + GAP
...
writeFileSync('release/sprite-preview.png', encodePng(sheetW, sheetH, px))
console.log(`release/sprite-preview.png ${sheetW}x${sheetH}`)
names.forEach((name, i) => {
  const col = i % COLS
  const row = Math.floor(i / COLS)
