...
// Official `fallblock` resolves each cell through the movement `check` —
// only stop/push/pull/still block; a faller drops straight through
// walk-over units like a bare flag.
test('step FALL passes through non-solid units', () => {
  const level: LevelData = {
    title: 'fall-through-flag',
    width: 5,
    height: 5,
    items: [
      createItem(1, 'baba', 2, 0, false),
      createItem(2, 'flag', 2, 2, false),
      createItem(3, 'baba', 0, 4, true),
      createItem(4, 'is', 1, 4, true),
      createItem(5, 'you', 2, 4, true),
      createItem(6, 'and', 3, 4, true),
      createItem(7, 'fall', 4, 4, true),
    ],
  }
...
test('step FALL lands on pushable units without pushing them', () => {
  const level: LevelData = {
    ...
    items: [
      createItem(1, 'baba', 2, 0, false),
      createItem(2, 'rock', 2, 2, false),
      createItem(3, 'baba', 0, 5, true),
      createItem(4, 'is', 1, 5, true),
      createItem(5, 'you', 2, 5, true),
      createItem(6, 'and', 3, 5, true),
      createItem(7, 'fall', 4, 5, true),
      createItem(8, 'rock', 0, 0, true),
      createItem(9, 'is', 0, 1, true),
      createItem(10, 'push', 0, 2, true),
    ],
  }
